...
- Build: configures CMake with `APP_VERSION` taken from the tag (strip leading `v`) and `UPDATE_FEED_URL` pointing to `https://github.com/<owner>/<repo>/releases/latest/download/update.json`.
...
4) The app checks updates from the feed URL `https://github.com/<owner>/<repo>/releases/latest/download/update.json`. Override at runtime with env `SCRIPT_TOOLBOX_UPDATE_URL` if you host it elsewhere.
...
- Configure with your desired version/feed:
  ...
  ```powershell
  cmake -S . -B build -G "MinGW Makefiles" -DAPP_VERSION=0.1.1 -DUPDATE_FEED_URL=https://github.com/<owner>/<repo>/releases/latest/download/update.json -D CMAKE_PREFIX_PATH=D:/app/qt/6.10.1/mingw_64
